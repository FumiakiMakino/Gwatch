...
try:
    ...
    fnum=inifnum
    iniadopi="-Patch 5 5 -FmDose 3.0 -Kv 200 -PixSize 1.12 -Cs 1.4 -FmRef 1 -FlipGain 1 -RotGain 1" #option for motionCor2
    adopi=iniadopi
    #Automatic 2D classificaiton
    iniMicro="50" #number of file for automatic 2D classification
    iniPix="1.12"  #Pixel size
    iniCs="1.4"    #Cs
    iniKv="200"    #kV
    ...
    with open("%s/.Gwatch_setting"%hpath,"w") as iniparaFile: 
       iniparaFile.write(inifname) #File name
       iniparaFile.write(inifnum)#stack number 
       iniparaFile.write(iniadopi) #option for motionCor2
       iniparaFile.write(iniMicro)  #number of file for automatic 2D classification
       iniparaFile.write(iniPix) #Pixel size
       iniparaFile.write(iniCs)  #Cs
       iniparaFile.write(iniKv)  #kV
       iniparaFile.write(iniDia)  #diameter of particle
       iniparaFile.write(refnameg)     #Gain referencefile
       iniparaFile.write(inibin)      #binning
       iniparaFile.write(iniadopigctf) #option for gctf
       iniparaFile.write(iniadopi2dcls) #option for 2d classification
...
def cal_2dcls(diameter, apix, kv, cs, binn, num, cmd2dcls):		
    ...
    cmd3rd="`which relion_preprocess` --i micrographs_all_gwatch%02d.star --coord_dir ./ --coord_suffix _automatch.star --part_star Extract/job000_%02d/particles.star --part_dir Extract/job000_%02d/ --extract --extract_size %d --scale %d --norm --bg_radius %d --white_dust -1 --black_dust -1 --invert_contrast > Extract/job000_%02d/run.out"%(num_bat,num_bat,num_bat,box_size,box_size/bin,int(box_size/bin*0.75/2),num_bat)

    #2D classification
    cmd4th0="mkdir -p Class2D/job000_%02d/"%(num_bat)
    cmd4th="mpirun -np %d `which relion_refine_mpi` --o Class2D/job000_%02d/run --i Extract/job000_%02d/particles.star --particle_diameter %d  %s > Class2D/job000_%02d/run.out"%(len(list4gpu)+1,num_bat,num_bat,(int(diameter*1.4)),cmd2dcls,num_bat) 
        ...
        if not event.is_directory and fnmatch.fnmatch(str(getname(event.src_path)),str(self.fname)):
            ...
            for i in range(120):#waiting copy for 6 min
                ...
                if(os.path.getsize(event.src_path) == file_size_bf): # If file size not changed, it means finishing file copy.	
                    ...
                    if ((str(getext(event.src_path)) != ".mrc") and  (str(getext(event.src_path)) != ".tiff") and (str(getext(event.src_path)) != ".tif")):
                        cmd_trs="newstack %s %s.mrc"%(event.src_path,getwholename(event.src_path))
                        cmd_rm="rm %s"%(event.src_path) #remove original image stack
                        subprocess.Popen(cmd_trs,shell=True)
...
"At the same time, Gwatch make a file \" micrographs_all_gwatch.star for Relion2 \" "))
    ...
    def message_help_option_2dcls(self):
        QMessageBox.information(self, "About option for Gctf", str("Set options for 2D classification by Relion2\n"
